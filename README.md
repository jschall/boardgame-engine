...
- **the CLI**: `node engine/bin/bg.js <parts|stats|balance|pack|jig|page|manual|lint|fit|ids|lettering|check|lasertime|concentric|all|new|doctor>`.
...
my-game/
  ...
  game.json        name, slug, sim, players, minutes, tagline, maker, year, code, geom, geom_files, fonts { family, R, SB, B }, manual.pages (manual.format 'letter' for a US Letter book), balance
  fonts/           the game's typefaces (the three weights named in game.json.fonts); the engine never picks a face
  ...
  page.css         the look: type, palette, loading screen (required; the engine's page.css is layout only)
...
### The rest
`page.js` exports `(game.json, parts.json) => { SHEET_GROUPS, banned, strip_ids, title }`; `pack.js` exports `ctx => { columns, place, after, flat_top, balance, clearance, thick }` (see `src/pack.js`); `jigs.js` exports `tray => { parts, items, bounds, meta, assembly }`; `manual/figures.js` exports `({ META, PARTS, S }) => { parts: [ids], renders: { name: { hash, width, height } }, extra }`; `manual/manual.js` lays out `#book` from `rules-data.js` (or the game's own `assets/*-data.js`, which `figures.extra` may write) and sets `window.__manualPageCount` (a multiple of four, the count in `game.json`). The book is the engine's 180 mm square unless `game.json` `manual.format` is `'letter'`: then the pages print as US Letter (the game's CSS sets `@page { size: Letter }` and lays each `.sheet` 8.5 x 11 in), the booklet is two pages a side on Tabloid landscape, and the page's reader shows Letter leaves.
...
boardgame-engine is free software under the Mozilla Public License 2.0 (`LICENSE`; SPDX `MPL-2.0`). Copyright (C) 2026 Jonathan Challinger. The `boardgame-create` skill in `plugins/` is part of it. The MPL is file-scoped, so a game built on the engine is its author's, under any license, including the page `bg page` builds, which embeds the engine's page files (`page/page.js`, `page/page.css`, the manual viewer, `lib/render3d.js`) as a Larger Work (MPL §3.3). What a published game owes for those embedded files: keep their license notices (the header comments, which the build carries into the page) and say where their source is (this repository at the engine commit the game pins; the notices name the repository). Changes to the engine's own files must be published under the MPL (a public fork is enough): that is how fixes come back. `starter/` (ORCHARD) and the skill's `templates/` are dedicated to the public domain under CC0 1.0 (`starter/LICENSE`), so a game made with `bg new` starts with no strings; `bg new` does not copy that file, and the new game's license is its author's to pick.

Third-party components with their own licenses: `lib/lasergeom.js` bundles JSTS (EPL-2.0 / EDL-1.0), Clipper2 (Boost Software License 1.0) and opentype.js (MIT); `fonts/` carries Fredoka under the SIL Open Font License 1.1 (`fonts/OFL.txt`). `npm install` brings playwright, sharp and pdf-lib under their own licenses.
