...
### 0. Brief
Capture: players, time, audience (adults by default), theme, machine (default xTool S1), stock with caliper readings per stock (two per stock; the owner's 3 mm basswood measured 2.67–2.92 mm, kerf 0.18), sheet budget (4), box style (the shoulder box), bought components the owner is happy to include, the product code if any. Ask now about 3D presence and token ergonomics if the brief leaves them open; otherwise decide. For a "recreate X" prompt: the mechanics are known; write `design.md` from them, with the numbers, and go on. `design.md` (ORCHARD's headings) is the single source of truth.
