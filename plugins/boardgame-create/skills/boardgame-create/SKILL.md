...
| the page, opening, demo, parts viewer, files | `page/page.js` `page/build.js` `lib/render3d.js` (machinery, layout) | `table.js` (the table layout and the animation), `page.js` (the sheet catalogue, banned words), **`page.css` + `fonts/`** (the look: type, colour, loading screen, the lot) |
| the print rulebook | `manual/build.js` `manual/assets.js` `manual/check.js` | `rules.js` (lid text and pages), `manual/manual.js` (layout), `manual/manual.css` (the book's face), `manual/figures.js` |
| the checks | `checks/*` | `qa_scenes.json`, `judge.json`, `preview_groups.json` |

`game.json` names the game (name, slug, sim, players, minutes, tagline, maker, year, product code, `geom_files`, `manual.pages`, **`fonts`: `{ family, R, SB, B }` filenames in `fonts/`**). The engine never picks a typeface or a palette. ORCHARD's `page.css` and `fonts/` are one example look; every game replaces them. BUMBLE's (`~/junk/boardgame/bumble` on the author's machine) show a hand-written generator with several stocks, a frame with shared cuts, and a rich table.
...
5b. **Trays** are the engine's `tray` kind (standard D10). Never grow the box to store pieces in pockets.
6. **The box**: the engine's shoulder box (`inner` 80–400 mm a side, or `[x, y]` a rectangle; `wall_h` 12–40 mm, default 24), packed inside closed (checked), a beautiful external engraving, the rules inside the lid, the safety warning on the lid, a medallion and the product code under the base. `bg jig` is the 24 mm raised-floor glue jig; a short flush tray may not get one.
...
11. **Instruction text is plain sentences** in everyday words, everywhere; the rulebook states every rule once, in play order, and nothing else: no slogan headings, no fun text, no reading guides; as many pages as the rules need and no more (`reference/manual.md`).
...
### 0. Brief
**Ask about the shop before any design work** (standard K4), in one message, only for what the prompt does not already say, and wait for the answer: the machine (make and model, laser type and power, bed size, whether it engraves); each material they will cut (kind, nominal thickness, and two caliper readings per stock, thinnest and thickest, if they have calipers; how many sheets and their size); kerf per stock only if they know it (the files are parametric: the page's stock panel takes the thickness readings and the kerf per stock and regenerates every sheet in the browser, and sheet 0's coupons measure the true kerf after the first cut; say so, do not make them guess, and do not let a missing reading stop the design: 0.18 mm and the nominal thickness range are the placeholders); the components they are happy to buy (dice, cubes, meeples, cards) if any. The geometry, the joints, the box size and the sheet budget all derive from these, so a design made before knowing them is made twice. Write the answers into `design.md` under `## The shop`.
...
### 4. Parts
Replace `art.js` and `geom.js` (the parts spec: tiles, tokens, cards, boards, plates, standees on keyed bases, pairs in + holes, trays with pockets for the pieces; the box's `cover`, `corner`, `setup_map`, `mark`, `lid`, and its `inner` (80–400 mm a side, or `[x, y]`) and `wall_h` (12–40, default 24) for the size the game needs), keeping the stocks measured in the brief. `bg parts` at the built stock, then `bg lint`; `bg fit '#<stock>lo=<lo>&<stock>hi=<lo>'` and at the thickest reading too; `bg lasertime` for the estimated engraving minutes per sheet and which elements the layout drew as concentric lines instead of rastering (standard D9: the machine software groups rasters, the estimate errs towards raster; give the owner the numbers to check against the machine's own estimate). Art rules in `reference/art.md`; numbers in `reference/mechanical.md`; process in `reference/manufacturing.md`.

### 5. Box, jig, packing (automatic)
`bg pack`, then `bg jig` when the box is the 24 mm raised-floor tray (a short flush tray may refuse the torsion jig). The packer's default piles (by kind and size) fit most games; `pack.js` places piles by hand (BUMBLE's frame). A box that does not fit its pieces is a design change (a smaller tile, a bigger INNER), not a packer change.

### 6. The page, the opening, the AI game
Replace `table.js` (the GameTable contract: the layout with the trays at `BOXO`/`LIDO`, `initTable`, `setBoardFromState`, `animateEvents` with one visible motion per event, `assertLegal`, `noteShown`, `syncBoard`, `finale`, the assemblies and groups) and `page.js`. **The page is a custom design for this game**: replace `page.css` and `fonts/` (and `game.json.fonts`) so the loading screen, the title, the type, the palette and the chrome are this game's, not ORCHARD's example. The engine's `page/page.css` is layout only (stage, nav, modals, variables). Then `bg page --no-manual && bg manual && bg page && bg check`. What goes on the page and what does not: `reference/page.md`.

### 7. The rulebook
Replace `rules.js` (the lid text: two columns and a footer; the pages: title, HTML with `<figure><img class="part" src="assets/<id>.svg">`), `manual/figures.js` (the parts and renders the figures use) and `manual/manual.js` if the starter's layout does not fit. `game.json.manual.pages` is the count (a multiple of four; four for a light game, more when the rules need it). `bg manual` builds, checks and verifies; `reference/manual.md` is the content, voice and design: every rule once, plain sentences, the setup as the real table drawn upright, no fun text. Rebuild the page (`bg page`) after every change so the owner can read the book, and run the plain-English audit before the rules judge.
...
- Playwright in real time for anything animated; `BG_GPU=1` for anything judged by eye or long (the checks are ten times faster on the GPU); press the button and watch the opening before believing it (`node engine/checks/scroll_sheet.js` makes the contact sheet of that opening).
