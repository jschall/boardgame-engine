...
"If every rule were explained succinctly, exactly once, how many pages would there be? Like 3 at most?" That is the book: every rule once, in the order the game is played, and nothing else. "The manual isn't the place for fun text. It's the place to avoid making the players read an encyclopedia and still feel stupid and frustrated." A rule stated twice will one day contradict itself; a paragraph that states no rule is weight. The page count is a multiple of four for the imposition and is whatever the rules need: a light game fits four US Letter pages (the cover, the pieces and the setup, the turn, the special piece and the end), and there is no hard limit when a page would otherwise drop a rule a player needs ("no hard and fast page limit btw if it is missing critical information"). The measure is completeness first, then brevity: read the finished book against the engine for every rule a player could ask about (which flaps can be used, an absent cat's corner wheel, what a full-pawed cat does with a treat) before cutting a page.
...
- **The layout audit** (`engine/manual/check.js`): no overflow, no orphan word, the page count matching `game.json.manual.pages`. When a page overflows, cut words that state no rule or shrink a figure (an inline `style="height:3in"` on the figure's svg), never the type and never a rule; if the rules do not fit, add four pages.
