...
node engine/checks/scroll_sheet.js <slug>.html /tmp/sheet.png 1600,900 20   # the opening as the reader sees it, 20 frames on one sheet (also 390,844 for phones)
...
| `checks/scroll_sheet.js` | the opening as the reader sees it: a real scroll through the track on the machine's GPU, 20 frames on one sheet (`node engine/checks/scroll_sheet.js <slug>.html /tmp/sheet.png 1600,900 20`; also 390,844 for phones); this is what caught the stage not sticking | playwright with `--use-angle=default --enable-gpu --ignore-gpu-blocklist`; frames in `/tmp/sheet-frames/` |
| `checks/opening_check.js` | anything passing through anything else while the box is unpacked: the opening frozen every 0.005 of the scroll with `#intro=`, the interior-sampling intersection test on the live scene at each; exits 1 on any pair (BUMBLE's went from 797 pairs to a handful; the named pairs say which rule of the flight plan is missing) | playwright on the GPU, about 4 minutes |
