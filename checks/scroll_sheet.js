...
(async () => {
  ...
  await pg.waitForFunction(() => window.__intro && window.__intro.live && window.__intro.ready); await pg.waitForTimeout(300); if (process.env.FAST_CAPTURE) await pg.addStyleTag({ content: '.vignette { display:none } .title h1, .title p { text-shadow:none }' });
  const frames = [];
  const shoot = async i => { const f = path.join(dir, `f${String(i).padStart(2, '0')}.png`); await pg.screenshot({ path: f, animations: 'disabled', timeout: 60000 }); frames.push(f); };
  await shoot(0);
  await pg.locator('#btn-box').click(); const t0 = Date.now();
  const OPEN_MS = 10000;   /* page.js's OPEN_MS */
  for (let i = 1; i < N; i++) {
    const at = OPEN_MS * i / (N - 1); await pg.waitForTimeout(Math.max(0, t0 + at - Date.now()));
    await shoot(i); console.log(`frame ${i} at ${((Date.now() - t0) / 1000).toFixed(1)} s, p ${(await pg.evaluate(() => window.__intro.p)).toFixed(3)}`);
  }
