...
console.log(`laser time model: raster ${M.raster_speed} mm/s in rows ${vf.pitch} mm apart (${vf.lines_per_inch} lines per inch) with ${M.accel} mm of run-in and run-out per row, objects merged into one sweep while that saves time; vector ${M.vector_speed} mm/s plus ${M.path_overhead} s per closed line; an element is drawn as concentric lines only when the saving is dramatic: the raster rows it forces cost at least ${M.min_gain} times its lines and the saving after that weighting is at least ${M.min_saving} s (elements under ${M.min_area} mm² are never considered; a part flagged raster in the spec never is).`);
let tb = 0, ta = 0, tv = 0;
...
for (const [name, r] of Object.entries(vf.sheets)) {
  ...
  console.log(`\n${name}${title}`);
  console.log(`  engraving: ${r.elements_total} elements${r.kept_raster ? ` (${r.kept_raster} flagged raster)` : ''}; raster ${mmss(r.raster_s_before)} as drawn, ${mmss(r.raster_s_after)} after ${r.elements} conversion${r.elements === 1 ? '' : 's'}; vector ${mmss(r.vector_s)} (${r.vector_mm} mm); estimated ${mmss(r.raster_s_after + r.vector_s)} instead of ${mmss(r.raster_s_before)}`);
  tb += r.raster_s_before; ta += r.raster_s_after; tv += r.vector_s;
  const rows = (r.decisions || []).filter(d => all || d.converted);
  if (rows.length) {
    console.log('  ' + ['decision', 'part', 'el', 'copies', 'area mm²', 'first copy on sheet (mm)', 'raster alone', 'sheet saving', 'vector', 'lines'].map((h, i) => h.padEnd([10, 22, 4, 7, 9, 30, 13, 13, 8, 5][i])).join(''));
    for (const d of rows) console.log('  ' + [d.converted ? 'vector' : 'raster', d.pid, String(d.element), String(d.copies), String(d.area_mm2), d.bbox.join(', '), `${d.raster_alone_s} s`, `${d.saving_s} s`, `${d.vector_s} s`, String(d.lines)].map((c, i) => c.padEnd([10, 22, 4, 7, 9, 30, 13, 13, 8, 5][i])).join(''));
