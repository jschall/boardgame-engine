...
(function (root, factory) {
  ...
  function floor_shape(F) {
    const t = F.FLOOR_TAB, e = F.FLOOR_EDGE, tw = F.TAB_W, lo = -F.EASE, hi = F.INNER0 + F.EASE, g = [sbox(lo - e, lo - e, hi + e, hi + e)];   /* INNER0: the drawn frame; the tray's ease is added once on each side */
    for (const c of F.TABS) g.push(sbox(c - tw / 2, lo - t, c + tw / 2, lo - e), sbox(c - tw / 2, hi + e, c + tw / 2, hi + t), sbox(lo - t, c - tw / 2, lo - e, c + tw / 2), sbox(hi + e, c - tw / 2, hi + t, c + tw / 2));
  ...
  function thumb_notch(rim_y, into, F) {
    const x = F.OUT / 2, off = NOTCH_R - NOTCH_DEPTH, f = NOTCH_FILLET;
  ...
  const slot_x = F => F.TABS.map(c => F.WALL_T + F.EASE + c);
  const slot_boxes = (y0, F) => unary_union(slot_x(F).map(x => sbox(x - F.SLOT_W / 2, y0, x + F.SLOT_W / 2, y0 + F.WALL_SLOT_H)));
  /** a wall: kind 'A' (S and N) owns the outer finger bands, 'B' (E and W) the middle band and the thumb notch; rim_top: the rim at y = 0 */
  function wall_shape(kind, rim_top, F) {
    const t = F.WALL_T, O = F.OUT, H = F.WALL_H, BAND = F.BAND, r = kind === 'B' ? F.CORE_RELIEF : 0;
    const core = r > 0 ? unary_union([sbox(t, BAND, O - t, 2 * BAND), sbox(t + r, 0, O - t - r, H)]) : sbox(t, 0, O - t, H);
    const owned = kind === 'A' ? [[0, BAND], [2 * BAND, 3 * BAND]] : [[BAND, 2 * BAND]];
    ...
    g = g.difference(slot_boxes(y0, F));
    if (kind === 'B') {
      const cut = rim_top ? thumb_notch(0.0, 1, F) : thumb_notch(H, -1, F);
      if (cut.distance(slot_boxes(y0, F)) < 3.0 || !sbox(BAND, -1, O - BAND, H + 1).contains(cut.intersection(sbox(0, 0, O, H)))) throw new Error('thumb notch runs into a slot or finger band');
      g = g.difference(cut);
    }
    return g;
  }
  ...
  function neck_shape(kind, F) {
    const t = F.NECK_T, H = F.NECK_H, NB = F.NECK_BAND, NO = F.NECK_OUT, s = t - F.NECK_FINGER;
    if (!(s >= 0)) throw new Error(`neck fingers ${F.NECK_FINGER} mm long are longer than the neck body's ${t} mm recess`);
    // the two bands at each end (grip bands) reach NECK_CL past the body on both ends, to the tray's real inside at the thickest reading;
    ...
    for (const b of (kind === 'A' ? [0, 2, 4] : [1, 3])) {
      const y1 = H - b * NB, y0 = y1 - NB, [p, q] = notched_band(y0, y1, H, F.BAND_NOTCH), reach = b === 2 ? -s : F.NECK_CL;
  ...
  function lid_outer_art(spec, INNER) {
    return memo(`box:lid_outer:${INNER}:${spec.title}:${spec.medal || 'circle'}:${spec.art_key || ''}`, () => {
      const W = INNER, cx = W / 2, g = [];
      const FR0 = 9.0, FR1 = 12.0, RC = 10.5, ros = [];
      for (const x of [RC, W - RC]) for (const y of [RC, W - RC]) ros.push(rosette(x, y, 5.2));
      const ros_sil = unary_union(ros.map(r => r[1]));
      const rules = unary_union([outline(sbox(FR0, FR0, W - FR0, W - FR0), 1.0), outline(sbox(FR1, FR1, W - FR1, W - FR1), 0.55)]);
      const frame = [rules.difference(ros_sil.buffer(-0.35))].concat(ros.map(r => r[0]));
      const RIB_H = 19.0, RIB_GAP = 5.5, warn_h = 12.0;
      const avail = (W - FR1 - 3.4 - warn_h - WARN_CLEAR) - (FR1 + 3.0);   // vertical room between the frame's top rule and the warning band
      ...
      const [warn, warn_box] = choking_warning(cx, W - FR1 - 3.4);
      ...
      return unary_union([K.finish(unary_union(frame.concat([comp, corners])), sbox(1, 1, W - 1, W - 1)), warn]);
    });
  }
  ...
  function lid_inner_art(spec, INNER) {
    return memo(`box:lid_inner:${INNER}:${JSON.stringify(spec.lid)}:${spec.art_key || ''}`, () => {
      ...
      const W = INNER, INSET = 5.0, LINE = 0.85, X0 = 10, X1 = W - 10, g = [outline(rrect(INSET, INSET, W - INSET, W - INSET, 2), LINE)];
      ...
      const top = y + 6, footer_h = R.footer ? 4.5 * Math.max(1, wrap(R.footer, 4.0, FONT.SB, X1 - X0).length) + 4 : 0, bottom = W - 11 - footer_h;
      ...
      if (!sbox(4.5, 4.5, W - 4.5, W - 4.5).contains(art)) throw new Error('lid engraving leaves the inner face / neck clearance');
      return art;
    });
  }
  ...
  function base_under_art(spec, INNER) {
    return memo(`box:base_under:${INNER}:${JSON.stringify([spec.title, spec.tagline, spec.players, spec.minutes, spec.year, spec.maker, spec.code])}:${spec.art_key || ''}`, () => {
      const W = INNER, g = [], has_code = !!(spec.code && spec.code.length), R = Math.min(52, W / 2 - 16), MX = has_code ? Math.max(R + 12, W / 2 - 16) : W / 2, MY = W / 2 - (has_code ? 8 : 0);
      ...
      let art = K.finish(unary_union(g), sbox(12, 12, W - 12, W - 12));
      if (has_code) {
        const code = upc_a(spec.code, W - 12 - 95 * 0.66, W - 12 - 17 - 7.4);
  ...
  function base_inner_art(spec, INNER) {
    return memo(`box:base_inner:${INNER}:${spec.title}:${spec.art_key || ''}`, () => {
      const W = INNER, g = [outline(sbox(6, 6, W - 6, W - 6), 0.8)];
      const map = typeof spec.setup_map === 'function' ? spec.setup_map(W) : null;
      if (map && !map.is_empty) { if (!sbox(7, 7, W - 7, W - 7).contains(map)) throw new Error('box.setup_map leaves its frame: keep it inside 7 mm of the floor edge'); g.push(map); }
      else g.push(ink(spec.title, 7, W / 2, W / 2 + 2.5, { font: FONT.B, spacing: 1.0 }));
      return K.finish(unary_union(g), sbox(1, 1, W - 1, W - 1));
    });
  }
  ...
  function wall_art_nominal(spec, side, half, F0) {
    return memo(`box:wall:${side}:${half}:${F0.OUT}:${spec.title}:${spec.art_key || ''}`, () => {
      if (typeof spec.wall_art === 'function') { const a = spec.wall_art(side, half, F0.OUT, F0.WALL_H); if (a && !a.is_empty) return a; }
      const O = F0.OUT, H = F0.WALL_H, g = [];
  ...
  function add_parts(reg, F, F0, spec) {
    const { keyed, add } = reg, INNER = F.INNER, FB = stock.tray(F, 'base'), FLD = stock.tray(F, 'lid');
    const mirrored = shape => K.back_art(shape, shape);
    const fl = floor_shape(FB), flL = floor_shape(FLD), flnom = memo(`box:floor:${INNER}`, () => floor_shape(F0));
    add('floor-base', fl);
    keyed('floor-base-under', null, 'box:floor-under', () => base_under_art(spec, INNER), { edge: flnom });
    keyed('floor-base-map', null, 'box:floor-map', () => base_inner_art(spec, INNER), { edge: mirrored(flnom), back: true });
    add('lid-cut', flL);
    keyed('lid-inner', null, 'box:lid-inner', () => lid_inner_art(spec, INNER), { edge: mirrored(flnom), back: true });
    keyed('lid-outer', null, 'box:lid-outer', () => lid_outer_art(spec, INNER), { edge: flnom });
    const keep = (FT, y0) => slot_boxes(y0, FT).buffer(EDGE + 0.2, { join_style: 'mitre' });
    const wall_post = (FT, y0) => { const sx = FT.OUT / F0.OUT; return { eng_post: g => K.clip_out(sx === 1 ? g : affinity.scale(g, sx, 1, 1, [0, 0]), keep(FT, y0)), eng_post_key: [sx, FT.SLOT_W, FT.WALL_T, FT.EASE, FT.WALL_SLOT_H, y0].join('|') }; };
    for (const side of 'SENW') {
      const kind = 'SN'.includes(side) ? 'A' : 'B';
      const edge_top = memo(`box:wall_edge:${kind}:${INNER}:top`, () => wall_shape(kind, true, F0)), edge_up = memo(`box:wall_edge:${kind}:${INNER}:up`, () => wall_shape(kind, false, F0));
      keyed(`wall-${side}-base`, wall_shape(kind, true, FB), `box:wall:${side}:base`, () => wall_art_nominal(spec, side, 'base', F0), Object.assign({ edge: edge_top }, wall_post(FB, F.SLOT_Y0)));
      keyed(`wall-${side}-lid-up`, wall_shape(kind, false, FLD), `box:wall:${side}:lid-up`, () => wall_art_nominal(spec, side, 'lid', F0), Object.assign({ edge: edge_up }, wall_post(FLD, F.FLOOR_UP)));
      keyed(`wall-${side}-lid`, wall_shape(kind, true, FLD), `box:wall:${side}:lid`, () => affinity.rotate(wall_art_nominal(spec, side, 'lid', F0), 180, [F0.OUT / 2, F0.WALL_H / 2]), Object.assign({ edge: edge_top }, wall_post(FLD, F.SLOT_Y0)));
    }
