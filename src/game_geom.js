...
// with register_fonts and set_store on it) and register the game's fonts/ on it. Both the parts CLI and its prewarm workers start here.
'use strict';
...
module.exports = function load_game(GAME_DIR) {
  ...
  const fonts = require('./game_fonts.js')(GAME_DIR, CFG);
  GAME.register_fonts({ files: fonts.files, roles: fonts.roles });
  return { CFG, GAME, lg: require(path.join(ENGINE, 'lib', 'lasergeom.js')), fonts };
};
