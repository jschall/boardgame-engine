...
async function main(GAME_DIR) {
  ...
  check(`the press PDF has ${PAGES} pages with 180 mm trim and 186 mm bleed boxes`, () => { assert.equal(pdf.getPageCount(), PAGES); for (const p of pdf.getPages()) { assert(Math.abs(p.getTrimBox().width - 180 * 72 / 25.4) < .01); assert(Math.abs(p.getBleedBox().width - 186 * 72 / 25.4) < .01); } });
  check('the booklet is US Letter landscape with every logical page exactly once', () => { assert.equal(bk.getPageCount(), PAGES / 2); for (const p of bk.getPages()) { assert.equal(p.getWidth(), 792); assert.equal(p.getHeight(), 612); } const imp = JSON.parse(fs.readFileSync(path.join(ROOT, 'qa/imposition.json'))); assert.deepEqual(imp.sideOrder.flat().slice().sort((a, b) => a - b), Array.from({ length: PAGES }, (_, i) => i + 1)); });
